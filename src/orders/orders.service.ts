...
export class OrdersService {
  ...
  async handlePaymentWebhook(payload: any) {
    ...
    if (!serverKey) {
      throw new BadRequestException('Server misconfiguration: MIDTRANS_SERVER_KEY not set');
    }
    ...
    if (isFailed) {
      ...
      return { message: `Payment ${transactionStatus}, stock released`, order: updatedOrder };
    }

    // transaction_status === 'pending' -> no action needed, the order remains PENDING.
    return { message: `Transaction status: ${transactionStatus}, no action taken`, order };
  }
}
